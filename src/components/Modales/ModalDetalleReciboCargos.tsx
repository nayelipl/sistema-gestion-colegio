...
const modalStyles: Record<string, React.CSSProperties> = {
  ...
  anuladoBadge: { background: "#fff5f5", border: "1px solid #fed7d7", borderRadius: "8px", padding: "8px", textAlign: "center" as const, color: "#c53030", fontWeight: "bold", marginBottom: "16px" },
  ...
  tachado: { textDecoration: "line-through", color: "#999" },
...
export function ModalDetalleReciboCargos({ isOpen, onClose, onPrint, recibo }: ModalDetalleReciboProps) {
  ...
  const estaAnulado = recibo.anulado === true;
    ...
    <div style={modalStyles.overlay}>
      <div style={modalStyles.modal}>
        ...
        <div style={modalStyles.body}>
          {/* Badge de anulado */}
          {estaAnulado && (
            <div style={modalStyles.anuladoBadge}>
              🚫 RECIBO ANULADO {recibo.motivoAnulacion && ` - Motivo: ${recibo.motivoAnulacion}`}
            </div>
          )}
          ...
          <div style={modalStyles.detalleGrid}>
            ...
            <div><strong>Estado:</strong> {estaAnulado ? "ANULADO" : "ACTIVO"}</div>
            {recibo.anulado && recibo.anuladoPor && (
              <>
                <div><strong>Anulado por:</strong> {recibo.anuladoPor}</div>
                <div><strong>Anulado en:</strong> {recibo.anuladoEn ? new Date(recibo.anuladoEn).toLocaleString("es-DO") : "—"}</div>
              </>
            )}
          </div>
